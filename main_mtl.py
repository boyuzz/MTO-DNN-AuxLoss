...
parser.add_argument("--alpha", type=str, default=0.0, help="percentage of copied model, Default: n=1.0")
...
# def mse_l1_loss(pred, label, size_average=True):
# 	return F.mse_loss(pred, label, size_average=size_average)+0.1*F.l1_loss(pred, label, size_average=size_average)
#
# def mse_ssime_loss(pred, label, size_average=True):
# 	return F.mse_loss(pred, label, size_average=size_average)+0.1*F.l1_loss(pred, label, size_average=size_average)

def main(run):
	# global steps, max_step
	print(opt)
	num_gpus = torch.cuda.device_count()
	...
	if opt.model == 'vdsr':
		model = vdsr.MTL_Net(num_channels=1, base_filter=64, num_residuals=18)
	else:
		model = edsr.MTL_Net(num_channels=1, base_filter=64, num_residuals=18, scale=opt.upscale_factor)

	print('===> Building criterions')
	criterion = [nn.MSELoss(), nn.L1Loss(), ssim_loss.SSIM(size_average=True)]

	print('===> Building optimizers')
	optimizer = optim.Adam(model.parameters(), lr=opt.lr)

	m1 = Train_op(model, optimizer, criterion, 'mtl', opt.lr, opt.upscale_factor, opt.cuda)
